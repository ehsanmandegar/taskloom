      ...
      <div className="grid">
        ...
        <section className="card workspace"><div className="card-title"><span>وضعیت اجرا</span><em>02</em></div>
          ...
          {!task?<div className="empty"><div><GitBranch size={36}/></div><h3>هنوز اجرایی شروع نشده</h3><p>اطلاعات تسک را وارد کن تا روند ساخت اینجا نمایش داده شود.</p></div>:<>
            ...
            <div className="delivery"><label>پیام commit<input value={task.commit_message||''} disabled={!passed||task.committed} onChange={e=>setTask({...task,commit_message:e.target.value})}/>{task.merge_request_url&&<a className="merge-request-link" href={task.merge_request_url} target="_blank" rel="noreferrer">مشاهده درخواست ادغام</a>}</label><div><button type="button" className="suggest-commit" aria-label="Generate commit message with Codex" disabled={!passed||task.committed||busy||!task.codex_thread_id} onClick={generateCommitMessage}><Sparkles size={16}/>ایجاد متن commit خودکار</button><button type="button" disabled={!passed||task.committed||busy} onClick={()=>action('commit',{message:task.commit_message})}><GitCommit size={16}/>{task.committed?'Commit شد':'Commit'}</button><button type="button" className="push" disabled={!task.committed||task.pushed||busy} onClick={()=>action('push')}><Upload size={16}/>{task.pushed?'Push شد':'Push branch'}</button><button type="button" className="merge-request" disabled={!task.pushed||!!task.merge_request_url||busy} onClick={()=>action('merge-request')}><GitPullRequest size={16}/>{task.merge_request_url?'MR ساخته شد':'Create MR'}</button></div></div>
          </>}
        </section>
      </div>
    </main>
