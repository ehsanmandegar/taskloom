        ...
        <section className="card workspace"><div className="card-title"><span>وضعیت اجرا</span><em>02</em></div>
          ...
          {!task?<div className="empty"><div><GitBranch size={36}/></div><h3>هنوز اجرایی شروع نشده</h3><p>اطلاعات تسک را وارد کن تا روند ساخت اینجا نمایش داده شود.</p></div>:<>
            ...
            <section className="chat" aria-label="Chat with Codex">
              ...
              <div className="chat-history">{task.messages.map((message,index)=><article className={`chat-message ${message.role}`} key={index}><b>{message.role==='user'?'شما':'Codex'}</b><p>{message.content}</p></article>)}</div>
              <form className="chat-composer" onSubmit={sendMessage}><textarea aria-label="پیام برای Codex" value={chatMessage} onChange={e=>setChatMessage(e.target.value)} rows="3" placeholder="ادامه بده، بهبود بده یا سؤال بپرس…" disabled={!task.codex_thread_id||running||busy}/><button type="submit" disabled={!chatMessage.trim()||!task.codex_thread_id||running||busy}><Send size={16}/> ارسال</button></form>
