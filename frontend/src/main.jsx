...
const initial = {project_path:'', guide_paths:'', task_id:'podw-205', request:'', test_command:'', mcp_server_name:'', mcp_failure_mode:'warning', git_provider:'gitlab'};
...
function App(){
  ...
  useEffect(()=>{fetch(`${API}/api/defaults`).then(async r=>{if(!r.ok)throw Error('خواندن پیش‌فرض‌ها ناموفق بود');const defaults=await r.json();const loaded={...initial,project_path:defaults.project_path||'',guide_paths:(defaults.guide_paths||[]).join('\n'),test_command:defaults.test_command||'',mcp_server_name:defaults.mcp_server_name||'',mcp_failure_mode:defaults.mcp_failure_mode||'warning'};setStartupDefaults(loaded);setForm(current=>current.project_path?current:{...current,...loaded})}).catch(e=>setNotice(e.message))},[]);
  ...
  useEffect(()=>{if(!task||['passed','failed','blocked'].includes(task.status)||typeof EventSource==='undefined')return;const events=new EventSource(`${API}/api/tasks/${task.id}/events`);events.onmessage=e=>{const next=JSON.parse(e.data);rememberTask(next);if(['passed','failed','blocked'].includes(next.status))events.close()};events.onerror=()=>events.close();return()=>events.close()},[task?.id,task?.status]);
  useEffect(()=>{ if(!task || ['passed','failed','blocked'].includes(task.status)) return; const timer=setInterval(async()=>{const r=await fetch(`${API}/api/tasks/${task.id}`); if(r.ok)rememberTask(await r.json())},1400); return()=>clearInterval(timer)},[task?.id,task?.status]);
  ...
  const running=task&&!['passed','failed','blocked'].includes(task.status), passed=task?.status==='passed', codexTyping=task&&['queued','running'].includes(task.status);
  const changeOutput=codexTyping&&task.live_output?task.live_output:(task?.diff||'در انتظار تغییرات…');
  const outputText=tab==='changes'?changeOutput:tab==='tests'?(task.test_output||'تست‌ها هنوز اجرا نشده‌اند…'):(task.live_response||task.summary||task.logs.join('\n')||'در حال تحلیل…');
    ...
    <main>
      ...
      <div className="grid">
        <form className="card form" onSubmit={start}>{gitProviderControl}
          ...
          <label>آدرس پروژه <small>پیش‌فرض Taskloom است؛ با تغییر مسیر، راهنما، تست و MCP پیش‌فرض آن پروژه پاک می‌شوند</small><input name="project_path" value={form.project_path} onChange={update} placeholder="C:\\work\\my-project" required/></label>
        ...
        <section className="card workspace"><div className="card-title"><span>وضعیت اجرا</span><em>02</em></div>
          ...
          {!task?<div className="empty"><div><GitBranch size={36}/></div><h3>هنوز اجرایی شروع نشده</h3><p>اطلاعات تسک را وارد کن تا روند ساخت اینجا نمایش داده شود.</p></div>:<>
            <div className="status"><span className={`status-icon ${task.status}`}>{passed?<CheckCircle2/>:['failed','blocked'].includes(task.status)?<XCircle/>:<span className="spinner"/>}</span><div><b>{task.step}</b><small><GitBranch size={13}/> {task.branch}</small>{task.mcp_server_name&&<small className={`mcp-state ${task.mcp_status}`}>MCP {task.mcp_server_name}: {task.mcp_message||task.mcp_status}</small>}</div><mark className={task.status}>{task.status}</mark></div>
            ...
            <section className="chat" aria-label="Chat with Codex">
              ...
              <div className="chat-history">{task.messages.map((message,index)=><article className={`chat-message ${message.role}`} key={index}><b>{message.role==='user'?'شما':'Codex'}</b><p dir="auto">{message.content}</p></article>)}{codexTyping&&(task.live_response?<article className="chat-message assistant live-response" role="status" aria-live="polite"><b>Codex در حال پاسخ است</b><p dir="auto">{task.live_response}</p><span className="typing-dots" aria-hidden="true"><i/><i/><i/></span></article>:<div className="chat-typing" role="status" aria-live="polite"><b>Codex در حال پاسخ است</b><span className="typing-dots" aria-hidden="true"><i/><i/><i/></span></div>)}</div>
